...
def feedback(request):
    ...
    if request.method == 'POST':
        ...
        d = request.POST['date']
        Send_Feedback.objects.create(user=request.user,username=u,msg=m,date=d)
        error=True
    d={'error':error}
...
def search_flight_by_name(request):
    if not request.user.is_staff:
        return redirect('login')
    data1=''
    d1=''
    if request.method == 'POST':
        try:
            d1= request.POST['flight_name']
            data1=Airline.objects.filter(flight_name=d1)
        except:
            pass
    d={'data':data1,'d1':d1}
